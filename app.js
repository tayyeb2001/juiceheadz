...
function renderMarquee() {
    ...
    // Duplicate array a few times to ensure enough content for smooth endless scroll
...
function getFilteredProducts() {
    ...
    return filtered.sort((a, b) => {
        switch(currentSort) {
            ...
            default: return 0; // Maintain original order
        }
    });
}
...
function renderProducts() {
    ...
    products.forEach((product, index) => {
        ...
        const message = encodeURIComponent(`Hi I would like to order ${product.name} - £${product.price}`);
        const waLink = `https://wa.me/${productsData.store.whatsapp.replace('+', '')}?text=${message}`;
        ...
        card.innerHTML = `
            ...
            <div class="card-content">
                ...
                <div class="product-footer">
                    ...
                    <a href="${waLink}" target="_blank" rel="noopener noreferrer" class="order-btn" title="Order via WhatsApp">
...
function setupEventListeners() {
    ...
    productGrid.addEventListener('click', (e) => {
        ...
        // If clicked on WhatsApp button, don't open modal
        if (orderBtn) return;
...
function getProductDescriptionKey(product) {
    ...
    // Exact or keyword matches based on rules
    ...
    if (name.includes('ignis') && name.includes('clen')) return 'clenbuterol'; // special case from data
    if (name.includes('clen')) return 'clenbuterol';
    if (name.includes('oxy 50') || name.includes('anadrol') || name.includes('nap 50')) return 'oxymetholone'; // or pharma_anadrol based on brand but oxy is generic
...
function openModal(product) {
    ...
    // Update CTA Link
    const message = encodeURIComponent(`Hi I would like to order ${product.name} - £${product.price}`);
    modalOrderBtn.href = `https://wa.me/${productsData.store.whatsapp.replace('+', '')}?text=${message}`;
